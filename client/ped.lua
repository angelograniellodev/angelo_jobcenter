CreateThread(function()
    for _, v in pairs(Config.Peds) do
        addNPC(v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], v[9])
    end
end)
    ...
    while Config.displayText do
        ...
        for _, v in pairs(Config.Peds) do
            ...
            if (distance < Config.displayDistance) then
                DrawText3D(v[1], v[2], v[3]+1, Config.displayColor .. v[7], 1.2, 2)
            else
                Citizen.Wait(950)
            end
        end
...
function addNPC(x, y, z, heading, hash, model, headingText, animation, base)
